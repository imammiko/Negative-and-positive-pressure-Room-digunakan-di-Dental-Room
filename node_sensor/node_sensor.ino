...
void loop() {
  //Serial.println(bmpValues());
  valuePressure = bmpValues();
  Serial.println(lastValuePressure);
  if (!(bmpValues() == lastValuePressure)) {
    payload.msNodeSensor=millis();
    lastValuePressure = bmpValues();
    payload.sensorBmpSensor = bmpValues();
    txKom(payload);
